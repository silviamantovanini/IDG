...
def auto_clip(msname, clip=3, width=10, column="DATA", mode="median",
	...
	channels_medians = np.full((nchans,), 0.)
	...
	for i in pbar:
		...
		if width*2+1 < nchans:
			...
			std = np.nanstd(
				channels_medians[window_min:window_max]
			)
	...
	if writeout is not None:
		with open(writeout, "w+") as f:
			f.write("channel,freq,fratio,median,std,window_median,window_std,flag\n")
...
def auto_clip_(msname, clip=3, width=10, column="DATA", mode="median",
	...
	print("")
	print("Found {} channels to flag in {} hours".format(len(flag_channels), datetime.now()-t1))
...
if __name__ == "__main__":
	...
	MeasurementSet = tables.table(args.msname, readonly=~args.noapply, ack=False)
