...
# Basic configuration
cluster="slurm"            # System-wide name of cluster, e.g. "garrawarla". This should match the name of the cluster, which can be checked 
...
export GXACCOUNT='OD-207757'               # The SLURM account jobs will be run under. e.g. 'pawsey0272'. Empty will not pass through a 
                                # corresponding --account=${GXACCOUNT} to the slurm submission. Only relevant if SLURM is tracking time usage 
export GXBASE="/scratch3/man318/software" # Path to base of GLEAM-X Pipeline where the repository was 'git clone' into including the name of the repository foldername, e.g. "/astro/mwasci/tgalvin/GLEAM-X-pipeline" 
export GXSCRATCH="/scratch3/man318/idg"     # Path to your scratch space used for processing on the HPC environment, e.g. /scratch
...
export GXCONTAINER="${GXBASE}/container/gleamx_container.img"  # Absolute path to the GLEAM-X singularity container, including the file name, e.g. "${GXSCRATCH}/gleamx.img"
...
export GXSTANDARDQ="ext"      # Slurm queue to submit tasks to, e.g. "workq". Available queues can be inspected using 'sinfo' on a system where the slurm schedular is available
...
export GXNCPUS=20               # Number of CPUs of each machine, e.g. 48. For tasks that have a 'core' like option this value is passed. 
export GXNLCPUS=20              # Number of logical CPUs on each machine, e.g. 48. This option may be phased out. 
export GXNPCPUS=20              # Number of physical CPUs on each machine, e.g. 24. This is meant to count only the physical cores available. 
...
export GXCOPYA='OD-207757'             # Account to submit obs_manta.sh job under, if time accounting is being performed by SLURM.
                            # Leave this empty if the job is to be submitted as the user and there is no time accounting.
export GXCOPYQ='ext'        # A required parameter directing the job to a particular queue on $GXCOPYM. Set as just the queue name, e.g. 'copyq'
export GXCOPYM='slurm'       # A required parameter directing the job to be submitted to a particular machine. Set as just the machine name, e.g. 'zeus'
