...
usage()
{
echo "obs_postidg.sh [-d dep] [-p project] [-a account] [-t] obsnum
...
script="${GXSCRIPT}/postidg_${obsnum}.sh"
cat "${GXBASE}/templates/postidg.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/postidg_${obsnum}.o%A"
error="${GXLOG}/postidg_${obsnum}.e%A"
...
for taskid in $(seq ${numfiles})
    ...
    then
        # record submission
        ${GXCONTAINER} track_task.py queue --jobid="${jobid}" --taskid="${taskid}" --task='postidg' --submission_time="$(date +%s)" \
