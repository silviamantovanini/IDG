...
usage()
{
echo "obs_mosaic.sh -p project [-d dep] [-q queue] [-t] -c channel -o list_of_observations.txt
bs
...
listbase=${listbase%%.*}
script="${GXSCRIPT}/mosaic_${listbase}_${channel}.sh"
...
output="${GXLOG}/mosaic_${listbase}_${channel}.o%A_%a"
error="${GXLOG}/mosaic_${listbase}_${channel}.e%A_%a"

chmod 755 "${script}"

# sbatch submissions need to start with a shebang
echo '#!/bin/bash' > "${script}.sbatch"
echo "srun --cpus-per-task=${GXNCPUS} --ntasks=1 --ntasks-per-node=1 singularity run /software/projects/mwasci/kross/GLEAM-X-pipeline/gleamx_swarp2.img ${script}" >> "${script}.sbatch"
