...
usage()
{
echo "obs_mosaic_left.sh -p project [-d dep] [-q queue] [-t] -c channel -o list_of_observations.txt
bs
...
listbase=${listbase%%.*}
script="${GXSCRIPT}/mosaic_left_${listbase}_${channel}.sh"

cat "${GXBASE}/templates/mosaic_left.tmpl" | sed -e "s:OBSLIST:${obslist}:g" \
                                      -e "s:CHANNEL:${channel}:g" \
                                      -e "s:BASEDIR:${base}:g" \
                                      -e "s:MOSAICDIR:${mosaicdir}:g" \
                                      -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/mosaic_left_${listbase}_${channel}.o%A_%a"
error="${GXLOG}/mosaic_left_${listbase}_${channel}.e%A_%a"
...
sub="sbatch  --begin=now+2minutes --array=0-4  --export=ALL  --time=10:00:00 --mem=230G -M ${GXCOMPUTER} --output=${output} --error=${error}"
