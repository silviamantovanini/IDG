...
usage()
{
echo "obs_aterm-all.sh [-d dep] [-p project] [-a account] [-z] [-t] obsnum
...
script="${GXSCRIPT}/aterm_all_${obsnum}.sh"
cat "${GXBASE}/templates/aterm_all.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:DEBUG:${debug}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/aterm_all_${obsnum}.o%A"
error="${GXLOG}/aterm_all_${obsnum}.e%A"
...
sub="sbatch --begin=now+2minutes --export=ALL --time=02:00:00 --mem=370G --account=mwasci --output=${output} --error=${error} --nodes=1 --cores=1 --gres=gpu:1 --cpus-per-gpu=38 --partition=gpuq"
