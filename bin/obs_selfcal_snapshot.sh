...
usage()
{
echo "obs_selfcal_snapshot.sh [-d dep] [-p project] [-a account] [-z] [-t] obsnum
...
script="${GXSCRIPT}/selfcal_snapshot_${obsnum}.sh"
cat "${GXBASE}/templates/selfcal_snapshot.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:DEBUG:${debug}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/selfcal_snapshot_${obsnum}.o%A"
error="${GXLOG}/selfcal_snapshot_${obsnum}.e%A"
...
sub="sbatch --begin=now --export=ALL  --time=12:00:00 --mem=${GXABSMEMORY}G -M ${GXCOMPUTER} --tmp=60GB --output=${output} --error=${error}"
