...
usage()
{
echo "wsc-all.sh [-d dep] [-p project] [-a account] [-z] [-t] obsnum
...
script="${GXSCRIPT}/wsc_all_${obsnum}.sh"
cat "${GXBASE}/templates/wsc_all.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:PROJECT:${project}:g" \
                                 -e "s:DEBUG:${debug}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/wsc_all_${obsnum}.o%A"
error="${GXLOG}/wsc_all_${obsnum}.e%A"
...
sub="sbatch --begin=now+5minutes --export=ALL --time=24:00:00 --mem=370G --account=mwasci --output=${output} --error=${error} --nodes=1 --gres=gpu:1 --cpus-per-gpu=38 --partition=gpuq --tmp=890GB"
