...
usage()
{
echo "obs_subcena.sh [-d dep] [-a account] [-t] obsnum
...
script="${GXSCRIPT}/subcena_${obsnum}.sh"

cat "${GXBASE}/templates/subcena.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                     -e "s:DATADIR:${datadir}:g" \
                                     -e "s:DEBUG:${debug}:g" \
                                     -e "s:PIPEUSER:${pipeuser}:g" > "${script}"


output="${GXLOG}/subcena_${obsnum}.o%A"
error="${GXLOG}/subcena_${obsnum}.e%A"
...
sub="sbatch --begin=now+1minutes --export=ALL  --time=12:00:00 --mem=50G -M ${GXCOMPUTER} --output=${output} --error=${error}"
