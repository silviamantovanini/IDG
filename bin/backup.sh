...
sub="sbatch --begin=now+1minutes --export=ALL --time=03:00:00 --mem-per-cpu=8G -M setonix --account=pawsey0272 --output=${output} --error=${error} --nodes=1 --cpus-per-task=4 --ntasks-per-node=1 --job-name=backup_${raregion}_${channel}" 
