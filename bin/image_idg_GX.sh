...
usage()
{
echo "image_idg_GX.sh [-d dep] [-p project] [-a account] [-z] [-t] obsnum
...
script="${GXSCRIPT}/image_IDG_GX_${obsnum}.sh"
cat "${GXBASE}/templates/image_idg_GX.tmpl" | sed -e "s:OBSNUM:${obsnum}:g" \
                                 -e "s:BASEDIR:${base}:g" \
                                 -e "s:DEBUG:${debug}:g" \
                                 -e "s:PIPEUSER:${pipeuser}:g" > "${script}"

output="${GXLOG}/image_IDG_GX_${obsnum}.o%A"
error="${GXLOG}/image_IDG_GX_${obsnum}.e%A"
...
sub="sbatch --begin=now+5minutes --export=ALL  --time=20:00:00 --mem=150G -M ${GXCOMPUTER} --tmp=150GB --output=${output} --error=${error}"
