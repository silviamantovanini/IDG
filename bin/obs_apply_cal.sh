...
echo "srun --cpus-per-task=1 --ntasks=1 --ntasks-per-node=1 singularity run -B /nvmetmp/ ${GXCONTAINER} ${script}" >> ${script}.sbatch
...
sub="sbatch --begin=now+5minutes  --export=ALL ${account} --time=05:00:00 --mem=24G --tmp=80GB -M ${GXCOMPUTER} --output=${output} --error=${error} "
