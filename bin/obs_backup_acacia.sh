...
sub="sbatch --begin=now+1minutes --export=ALL --time=15:00:00 --mem=180G -M garrawarla --account=mwasci --output=${output} --error=${error} --cpus-per-task=4 --ntasks-per-node=1 --job-name=backup_acacia_${raregion}_${channel}" 
